...
// Local calendar date as YYYY-MM-DD. en-CA is the shortest way to get ISO
// order out of toLocaleDateString without composing it by hand.
...
export const bookingSchema = yup.object({
  ...
  // Collections can't be booked for a date that has already passed. The one
  // exception is editing an existing booking: its original date may well be
  // in the past by now, and re-saving an unrelated typo must not be blocked
  // by it. Pass { context: { originalDate } } to allow exactly that value
  // through — changing it still requires today or later.
  collection_date: yup
    .string()
    .matches(/^\d{4}-\d{2}-\d{2}$/, "Enter a valid collection date.")
    .test("not-in-the-past", "Collection date cannot be in the past.", function (value) {
      if (!value) return true;
      const original = this.options?.context?.originalDate;
      if (original && value === original) return true;
